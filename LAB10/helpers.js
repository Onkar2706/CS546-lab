...
const validate = {
  ...
  checkIfString: (inp) => {
    ...
    if (typeof inp !== "string" || inp.trim().length <2 || inp.trim().length>25)
      throw "Error: Input parameter must be non-empty string";
  },
  checkIfUsername: (inp) => {
    validate.checkIfProperInput(inp);
    if (typeof inp !== "string" || inp.trim().length <5 || inp.trim().length>=10)
      throw "Error: Input parameter should have charcters between 5 and 10 ";
  },
  checkIfthemePreference: (inp) => {
    validate.checkIfProperInput(inp);
    if (typeof inp !== "string" || inp.trim()<=0 || inp.trim().toLowerCase() =="light" || inp.trim().toLowerCase() =="dark")
      throw "Error: input dhould be either light or dark ";
  },
  checkIfRole: (inp) => {
    validate.checkIfProperInput(inp);
    if (typeof inp !== "string" || inp.trim()<=0 || inp.trim().toLowerCase() =="admin" || inp.trim().toLowerCase() =="user")
      throw "Error: input dhould be either light or dark ";
  },
  checkIfFavoriteQuote: (inp) => {
    validate.checkIfProperInput(inp);
    if (typeof inp !== "string" || inp.trim().length <20 || inp.trim().length>=255)
      throw "Error: Input parameter should have charcters between 5 and 10 ";
  },
