$(document).ready(function() {
    ...
    function displaySearchResults(movies) {
      ...
      movies.forEach(movie => {
        const li = $('<li>');
        const a = $('<a>').attr('href', 'javascript:void(0)').attr('data-id', movie.imdbID).text(movie.Title);
        li.append(a);
    ...
    function displayMovieDetails(movie) {
      const html = `
        <article>
          <h1>${movie.Title}</h1>
          <img src="${movie.Poster !== 'N/A' ? movie.Poster : 'no_image.jpeg'}" alt="${movie.Title} Poster">
          <h2>Plot</h2>
          ...
          <section>
            <h4>Cast and Crew</h4>
            <p><strong>Director:</strong> ${movie.Director !== 'N/A' ? movie.Director : 'N/A'}</p>
            <p><strong>Writer:</strong> ${movie.Writer !== 'N/A' ? movie.Writer : 'N/A'}</p>
            <p><strong>Cast:</strong> ${movie.Actors !== 'N/A' ? movie.Actors : 'N/A'}</p>
          </section>
          <section>
            <h4>Ratings</h4>
